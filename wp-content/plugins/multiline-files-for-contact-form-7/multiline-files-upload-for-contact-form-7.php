...
function mfcf7_zl_multilinefile_shortcode_handler($tag)
{
	...
	$html .= '<p><lable><span class="wpcf7-form-control-wrap" data-name="'.$btntagname.'"><input type="button" name="'.$btntagname.'" class="button button-primary qbutton" href="javascript:void(0);" id="mfcf7_zl_add_file" value="' . $upload_label . '"></span></lable></p>';
...
function zlchange_attachments($cf7, &$abort, $object)
{
	...
	$single_img = '';
	if ($submission) {
		$multitags = $submission->get_contact_form()->scan_form_tags(array(
			'type' => 'multilinefile',
		));
