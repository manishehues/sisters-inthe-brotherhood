...
class Super_Simple_Slider_Post_Type {
	...
	public function __construct( $parent, $post_type = '', $plural = '', $single = '', $description = '', $options = array() ) {
		...
		add_action( 'save_post', array( $this, 'save_slider_break_points'), 10, 2 ); 
	...
	*/
		...
		<div class="otb-postbox-container">
			...
			<div class="footer">
				<?php 
					...
					$slides_loop = get_post_meta( $post->ID, 'slides_loop', true );

					$slides_loop =  $slides_loop ? 'checked="checked"' : '';

					$slides_perView = get_post_meta( $post->ID, 'slides_perView', true );
					$slides_per_group = get_post_meta( $post->ID, 'slides_per_group', true );

					$slides_perView_1 = get_post_meta( $post->ID, 'slides_perView_1', true );
					$slides_per_group_1 = get_post_meta( $post->ID, 'slides_per_group_1', true );

					$slides_perView_2 = get_post_meta( $post->ID, 'slides_perView_2', true );
					$slides_per_group_2 = get_post_meta( $post->ID, 'slides_per_group_2', true );

					$slides_perView_3 = get_post_meta( $post->ID, 'slides_perView_3', true );
					$slides_per_group_3 = get_post_meta( $post->ID, 'slides_per_group_3', true );
				
				?>
				<table style="width:75%; border-spacing: 30px">
					<thead>
						<caption><h1>Slider Settings</h1></caption>
					<thead>
					<tbody>

						<tr>
							<td><code >Slides Loop</code></td>
							<td>	
								<input type="checkbox" name="slides_loop" <?php echo $slides_loop ?> >&nbsp;&nbsp;
								<i>check if loop the slider</i> 
							</td>
						</tr>
						
						<tr>
							<td><code >Default width < 768</code></td>
							<td>
								<code >Slides PerView </code>&nbsp;
								<input type="text" name="slides_perView" placeholder="enter value" value="<?php echo $slides_perView ?>">
							</td>
							<td>
								<code >Slides Per Group </code>&nbsp;
								<input type="text" name="slides_per_group" placeholder="enter value" value="<?php echo $slides_per_group ?>">
							</td>
						</tr>
							
						<tr>
							<td><code >width is >= 768px</code></td>
							<td>
								<code >Slides PerView </code>&nbsp;	
								<input type="text" name="slides_perView_1" placeholder="enter value" value="<?php echo $slides_perView_1 ?>">
							</td>
							<td>
								<code >Slides Per Group </code>&nbsp;	
								<input type="text" name="slides_per_group_1" placeholder="enter value" value="<?php echo $slides_per_group_1 ?>"></td>
						</tr>	
						
						<tr>
							<td><code >width is >= 1024px</code></td>
							
							<td>
								<code >Slides PerView </code>&nbsp;	
								<input type="text" name="slides_perView_2" placeholder="enter value" value="<?php echo $slides_perView_2 ?>" >
							</td>
							
							<td>
								<code >Slides Per Group </code>&nbsp;		
								<input type="text" name="slides_per_group_2" placeholder="enter value" value="<?php echo $slides_per_group_2 ?>">
							</td>
						</tr>
							
						
						<tr>
							<td><code >width is >= 1280px</code></td>
							
							<td>
								<code >Slides PerView </code>&nbsp;	
								<input type="text" name="slides_perView_3" placeholder="enter value" value="<?php echo $slides_perView_3 ?>">
							</td>
							
							<td>
								<code >Slides Per Group </code>&nbsp;
								<input type="text" name="slides_per_group_3" placeholder="enter value" value="<?php echo $slides_per_group_3 ?>" ></td>
						</tr>
	...
	public function save_slider_break_points($postId){

		$slides_loop 		= sanitize_text_field($_REQUEST['slides_loop']);

		$slides_perView 	= sanitize_text_field($_REQUEST['slides_perView']);
		$slides_per_group 	= sanitize_text_field($_REQUEST['slides_per_group']);

		$slides_perView_1 	= sanitize_text_field($_REQUEST['slides_perView_1']);
		$slides_per_group_1 = sanitize_text_field($_REQUEST['slides_per_group_1']);

		$slides_perView_2 	= sanitize_text_field($_REQUEST['slides_perView_2']);
		$slides_per_group_2 = sanitize_text_field($_REQUEST['slides_per_group_2']);

		$slides_perView_3 	= sanitize_text_field($_REQUEST['slides_perView_3']);
		$slides_per_group_3 = sanitize_text_field($_REQUEST['slides_per_group_3']);

		update_post_meta( $postId, 'slides_loop', $slides_loop );

		update_post_meta( $postId, 'slides_perView', $slides_perView );
		update_post_meta( $postId, 'slides_per_group', $slides_per_group );
		
		update_post_meta( $postId, 'slides_perView_1', $slides_perView_1 );
		update_post_meta( $postId, 'slides_per_group_1', $slides_per_group_1 );

		update_post_meta( $postId, 'slides_perView_2', $slides_perView_2 );
		update_post_meta( $postId, 'slides_per_group_2', $slides_per_group_2 );

		update_post_meta( $postId, 'slides_perView_3', $slides_perView_3 );
		update_post_meta( $postId, 'slides_per_group_3', $slides_per_group_3 );
